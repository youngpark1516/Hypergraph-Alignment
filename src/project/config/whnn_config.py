...
dataset_configs = {
    "partnet": config_dir / "whnn_partnet.yaml",
...
loss_arg.add_argument("--weight_spectralmatching", type=float, default=0.0)
loss_arg.add_argument("--weight_hypergraph", type=float, default=0.0)
...
misc_arg.add_argument("--eval_num", type=int, default=0, help="cap number of validation files evaluated; 0 uses all")
misc_arg.add_argument("--eval_seed", type=int, default=0, help="random seed for eval_num file sampling")
misc_arg.add_argument("--generation_method", type=str, default="spectral-2", choices=["spectral-2", "spectral", "greedy"])
misc_arg.add_argument("--generation_min_score", type=float, default=0.4,
                      help="absolute minimum spectral score for spectral generation methods")
misc_arg.add_argument("--generation_min_confidence", type=float, default=0.9,
...
# WHNN configurations
whnn_arg = add_argument_group("WHNN")
whnn_arg.add_argument("--dropout", type=float, default=0.5)
whnn_arg.add_argument("--input_dropout", type=float, default=0.0)
whnn_arg.add_argument("--All_num_layers", type=int, default=2)
whnn_arg.add_argument("--MLP_num_layers", type=int, default=1)
whnn_arg.add_argument("--MLP_hidden", type=int, default=128)
whnn_arg.add_argument("--Classifier_num_layers", type=int, default=1)
whnn_arg.add_argument("--Classifier_hidden", type=int, default=128)
whnn_arg.add_argument("--aggregate", type=str, default="mean", choices=["sum", "mean"])
whnn_arg.add_argument("--normalization", type=str, default="ln")
whnn_arg.add_argument("--deepset_input_norm", type=str2bool, default=False)
whnn_arg.add_argument("--heads", type=int, default=1)
whnn_arg.add_argument("--MLP3_num_layers", type=int, default=0)
whnn_arg.add_argument("--restart_alpha", type=float, default=0.5)
whnn_arg.add_argument("--proc_type", type=str, default="MLP", choices=["MLP", "Id", "SAB", "ISAB"])
whnn_arg.add_argument("--pooling_type", type=str, default="FPSWE", choices=["DeepSet", "PMA", "FPSWE", "LPSWE"])
whnn_arg.add_argument("--apprepset_n_anchors", type=int, default=32)
whnn_arg.add_argument("--isab_num_inds", type=int, default=32)
whnn_arg.add_argument("--sharing", type=str2bool, default=False)
...
def get_config():
    ...
    experiment_id = f"WHNN_{args.dataset}_{time.strftime('%m%d%H%M')}"
    ...
    if args.dataset != args.dataset and args.exp_id == experiment_id:
        new_exp_id = f"WHNN_{args.dataset}_{datetime.now().strftime('%m%d%H%M%S_%f')}"
